...
const controlRecipes = async function() {
    try {
        ...
        (0, _resultsViewJsDefault.default).update(_modelJs.loadSearchResultsPerPage());
...
const controlSearchResults = async function() {
    try {
        ...
        // 3) Render Results :
        (0, _resultsViewJsDefault.default).render(_modelJs.loadSearchResultsPerPage());
...
const controlPages = function(goToPage) {
    ...
    (0, _resultsViewJsDefault.default).render(_modelJs.loadSearchResultsPerPage(goToPage));
...
const controlAddRecipe = async function(newRecipe) {
    try {
        ...
        // Close form Window :
        setTimeout(function() {
            if (!(0, _addRecipeViewJsDefault.default)._overlay.classList.contains("hidden")) (0, _addRecipeViewJsDefault.default)._toggle();
        }, 2500);
    } catch (err) {
        console.error(err);
...
// Subscriber :
const init = function() {
    (0, _bookmarksViewJsDefault.default).addHandlerLoad(controlLoading);
    ...
    (0, _paginationViewJsDefault.default).addHandlerPagination(controlPages);
...
parcelHelpers.export(exports, "loadSearchResultsPerPage", ()=>loadSearchResultsPerPage);
...
const state = {
    ...
    search: {
        ...
        result: [],
        page: 1,
        resultsPerPage: (0, _configureJs.RES_PER_PAGE)
    },
    bookmarks: []
};
...
const loadRecipe = async function(id) {
    try {
        const data = await (0, _helpersJs.AJAX)(`${(0, _configureJs.API_URL)}/${id}?key=${(0, _configureJs.KEY)}`);
...
const loadSearchResults = async function(query) {
    try {
        ...
        const data = await (0, _helpersJs.AJAX)(`${(0, _configureJs.API_URL)}?search=${query}&key=${(0, _configureJs.KEY)}`);
        console.log(data);
...
const loadSearchResultsPerPage = function(page = state.search.page) {
...
const uploadRecipe = async function(newRecipe) {
    try {
        ...
        const data = await (0, _helpersJs.AJAX)(`${(0, _configureJs.API_URL)}?key=${(0, _configureJs.KEY)}`, newRecipeObject);
        state.recipe = createRecipeObject(data);
...
},{"./configure.js":"l0Gg8","./helpers.js":"hGI1E","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"l0Gg8":[function(require,module,exports) {
// Folder creationn Date : 8 Sept 2024
// In this file we are declare and export the constant variables or values that is used again and again in the entire project so that if we have to make major changes regarding that variables so we are only change in this file !
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const KEY = "b7bce8d8-8a58-4e37-998f-ff84c1999c90";
...
// folder creation Date : 8 sept 2024
// In this file we define the functions that is reused again and again in this projects !
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const AJAX = async function(url, uploadData) {
    try {
        ...
        const res = await Promise.race([
            fetchPro,
            timeout((0, _configureJs.TIMEOUT_SEC))
        ]);
...
class RecipeView extends (0, _viewJsDefault.default) {
    ...
    addHandlerRender(Subscriber) {
        [
            "hashchange",
            "load"
        ].forEach((ev)=>window.addEventListener(ev, Subscriber));
    }
    ...
    _generateMarkupIngredient(ing) {
        let quantity = `${ing.quantity}`.includes("null") ? `${ing.quantity}`.replace("null", "") : `${+ing.quantity}`;
...
class paginationView extends (0, _viewJsDefault.default) {
    ...
    _generateMarkup() {
        ...
        if (currPage === 1 && numPages > 1) return this._generateMarkupNext(currPage);
        // 2 : We are at 1st page and there is no other page exist .
        if (currPage === 1 && numPages === 1) return ""; // grapes !
        // 3 : We are at last page
        if (currPage === numPages && numPages > 1) return this._generateMarkupPrev(currPage);
        // 4 : Other Pages
        if (currPage < numPages) return `${this._generateMarkupPrev(currPage)} ${this._generateMarkupNext(currPage)}`;
    }
